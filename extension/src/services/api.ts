...
export async function optimizeShoppingList(
  ...
  return {
    ...
    singleStoreBest: {
      retailerId: isGermany ? 'netto-de' : isUK ? 'tesco-uk' : 'metro-pk',
      retailerName: isGermany ? 'Netto Marken-Discount' : isUK ? 'Tesco Extra' : 'Metro Cash & Carry',
      storeAddress: `Nearest ${isGermany ? 'Netto' : isUK ? 'Tesco' : 'Metro'} Branch`,
      distanceKm: 0.8,
      ...
      itemBreakdown: items.map(i => ({
        item: i.name,
        productName: i.name,
        brand: isGermany ? 'Netto Bio' : isUK ? 'Tesco' : 'Metro Deal',
    ...
    multiStoreOptimal: {
      ...
      stores: [
        ...
        {
          retailerId: isGermany ? 'rewe-de' : isUK ? 'sainsburys-uk' : 'al-fatah',
          retailerName: isGermany ? 'REWE City' : isUK ? 'Sainsbury\'s' : 'Al-Fatah Supermarket',
