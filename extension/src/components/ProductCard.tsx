...
import { ExternalLink, Navigation, CheckCircle2, Clock } from 'lucide-react';
...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 p-3.5 shadow-xs hover:shadow-md transition-shadow relative">
      ...
      {/* Verification Status & Fetched Timestamp */}
      <div className="mt-2.5 pt-2 border-t border-slate-100 flex flex-col gap-1.5 text-[10px] text-slate-500">
        <div className="flex items-center justify-between gap-1">
          <div className="flex items-center gap-1 text-slate-600 font-medium">
            <Clock className="w-3 h-3 text-emerald-600 shrink-0" />
            <span>
              Fetched: <strong className="text-slate-800 font-semibold">{offer.lastVerified ? formatTime(offer.lastVerified) : 'Live Feed (Today)'}</strong>
            </span>
          </div>
        ...
        <div className="flex items-center justify-between gap-1 pt-1 border-t border-slate-50">
          {offer.validUntil ? (
            ...
            <span className="text-[10px] text-emerald-600 font-medium">Active Deal</span>
          )}
